...
class CrossCellProductsOptionsController: ced_VendorBaseClass,UITableViewDataSource,UITableViewDelegate,UIGestureRecognizerDelegate {
    ...
    @IBOutlet weak var crossSellTableView: UITableView!
    
    var baseURL = String();
    let screenSize: CGRect = UIScreen.main.bounds;
    let relatedProCellHeight = CGFloat(235);
    ...
    override func viewDidLoad() {
        ...
        topLabel.text = "CROSS SELL Products".localized
        leftButton.setTitle("Save".localized, for: UIControl.State());
        leftButton.addTarget(self, action: #selector(CrossCellProductsOptionsController.saveFunction(_:)), for: UIControl.Event.touchUpInside);
        
        middleButton.addTarget(self, action: #selector(CrossCellProductsOptionsController.filterButtonPressed(_:)), for: UIControl.Event.touchUpInside);
        
        rightButton.setTitle("Continue".localized, for: UIControl.State());
        rightButton.addTarget(self, action: #selector(CrossCellProductsOptionsController.continueFunction(_:)), for: UIControl.Event.touchUpInside);
        ...
        self.crossSellTableView.delegate = self;
        self.crossSellTableView.dataSource = self;
        
        self.fetchRelatedProductListing(self.pages);
        //showPageDropDown.addTarget(self, action: #selector(CrossCellProductsOptionsController.showPageNation(_:)), for: .touchUpInside)
    ...
    {
        let cell = crossSellTableView.dequeueReusableCell(withIdentifier: "longSingleCell", for: indexPath) as! LongSingleTableViewCell;
        ...
        for (val) in products
        {
            ...
            relatedProductView.selectionButton.addTarget(self, action: #selector(CrossCellProductsOptionsController.makeProductSelected(_:)), for: UIControl.Event.touchUpInside);
    ...
    @objc func fetchRelatedProductListing(_ page:Int)
    {
        ...
        let baseURL = "vproductapi/vproducts/crossSell/page/\(page)";
            ...
            if(requestUrl == "vendorapi/vproducts/update/")
            {
                ...
                if(jsonData["data"]["success"].stringValue != "true")
                {
                    ...
                    if pages == 1{
                        ...
                        self.crossSellTableView.setEmptyMessage(jsonData["data"]["message"].stringValue)
                    }
                    
                    self.crossSellTableView.reloadData()
                    return;
                }
                
               
//                self.totalProducts.text = "Total Products : " + jsonData["data"]["count"].stringValue
                ...
                self.crossSellTableView.restore()
                self.crossSellTableView.reloadData();
            }
        }
    }
    ...
    @objc func filterButtonPressed(_ sender:UIButton)
    {
        ...
        longFilterPopupView.value5.addTarget(self, action: #selector(CrossCellProductsOptionsController.showProductStatusDropdown(_:)), for: UIControl.Event.touchUpInside);
        
        longFilterPopupView.value6.setTitle(value6, for: UIControl.State());
        longFilterPopupView.value6.addTarget(self, action: #selector(CrossCellProductsOptionsController.showProductTypeDropdown(_:)), for: UIControl.Event.touchUpInside);
        
        longFilterPopupView.value7.setTitle(value7, for: UIControl.State());
        longFilterPopupView.value7.addTarget(self, action: #selector(CrossCellProductsOptionsController.showAttributeSetDropdown(_:)), for: UIControl.Event.touchUpInside);
        ...
        longFilterPopupView.leftButton.addTarget(self, action: #selector(CrossCellProductsOptionsController.applyFilterTapped(_:)), for: UIControl.Event.touchUpInside);
        
        longFilterPopupView.rightButton.setTitle("Reset", for: UIControl.State());
        longFilterPopupView.rightButton.addTarget(self, action: #selector(CrossCellProductsOptionsController.resetFilterTapped(_:)), for: UIControl.Event.touchUpInside);
    ...
    @objc func addgesturesTohideView(_ v:UIView)
    {
        ...
        let tapGesture = UITapGestureRecognizer (target: self, action:#selector(CrossCellProductsOptionsController.hideView(_:)))
        v.addGestureRecognizer(tapGesture);
        tapGesture.delegate=self;
        
        let upSwipe = UISwipeGestureRecognizer(target: self, action: #selector(CrossCellProductsOptionsController.hideView(_:)));
        upSwipe.direction = UISwipeGestureRecognizer.Direction.up;
        v.addGestureRecognizer(upSwipe)
        upSwipe.delegate=self;
        
        let downSwipe = UISwipeGestureRecognizer(target: self, action: #selector(CrossCellProductsOptionsController.hideView(_:)));
        downSwipe.direction = UISwipeGestureRecognizer.Direction.down;
        v.addGestureRecognizer(downSwipe)
        downSwipe.delegate=self;
        
        let rightSwipe = UISwipeGestureRecognizer(target: self, action: #selector(CrossCellProductsOptionsController.hideView(_:)));
        rightSwipe.direction = UISwipeGestureRecognizer.Direction.right;
        v.addGestureRecognizer(rightSwipe)
        rightSwipe.delegate=self;
        
        let leftSwipe = UISwipeGestureRecognizer(target: self, action: #selector(CrossCellProductsOptionsController.hideView(_:)));
    ...
    @objc func updateProductInfo(_ proceed:Bool)
    {
        ...
        if(strToPost != "")
        {
            postString += "&crossellproducts="+strToPost;
        }
    ...
    @objc func switchToNextView()
    {
        ...
        {
            if  let viewcontrollor = self.storyboard?.instantiateViewController(withIdentifier: "bundleItemsViewController") as? BundleItemsViewController{
            //edit time
            viewcontrollor.isEditCase = isEditCase;
            //edit time
            viewcontrollor.selectedProductTypeId = selectedProductTypeId;
