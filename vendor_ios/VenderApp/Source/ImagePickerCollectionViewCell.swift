...
//        ]
        ...
        urlDataTask = session.dataTask(with: url) { [weak self] (data, response, error) in
    ...
    private func addOverlay(_ animated: Bool) {
        ...
        overlayImageView.image = selectionImage ?? UIImage(named: "ic_checkmark", in: Bundle.podBundle(forClass: type(of: self).self), compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
        overlayImageView.tintColor = selectionImageTintColor
