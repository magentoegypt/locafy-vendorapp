...
@objc open class ScrollableGraphView: UIScrollView, UIScrollViewDelegate, ScrollableGraphViewDrawingDelegate {
    ...
    open var lineJoin = convertFromCAShapeLayerLineJoin(CAShapeLayerLineJoin.round)
    /// The line caps. Takes any of the Core Animation LineCap values.
    open var lineCap = convertFromCAShapeLayerLineCap(CAShapeLayerLineCap.round)
    ...
    open var shouldDrawBarLayer = false
    ...
    open var backgroundFillColor = UIColor.white
    ...
    open var shouldAutomaticallyDetectRange = false
    ...
    open var shouldRangeAlwaysStartAtZero = false // Used in conjunction with shouldAutomaticallyDetectRange, if you want to force the min to stay at 0.
    ...
    open var shouldDrawDataPoint = true
    ...
    open var shouldAdaptRange = false
    ...
    open var shouldAnimateOnAdapt = true
    ...
    /// If adaptAnimationType is set to .Custom, then this is the easing @objc function you would like applied for the animation.
    ...
    open var shouldAnimateOnStartup = true
    
    // Reference Lines
    // ###############
    
    /// Whether or not to show the y-axis reference lines and labels.
    open var shouldShowReferenceLines = true
    /// The colour for the reference lines.
    open var referenceLineColor = UIColor.black
    ...
    open var shouldAddLabelsToIntermediateReferenceLines = true
    ...
    open var shouldAddUnitsToIntermediateReferenceLineLabels = false
    ...
    open var referenceLineLabelColor = UIColor.black
    
    /// Whether or not to show the units on the reference lines.
    open var shouldShowReferenceLineUnits = true
    ...
    open var shouldShowLabels = true
    ...
    open var dataPointLabelColor = UIColor.black
    ...
    fileprivate func createLinePath() -> UIBezierPath {
        ...
        let numberOfPoints = min(data.count, activePointsInterval.upperBound)
        ...
        for i in activePointsInterval.lowerBound ..< numberOfPoints {
  ...
  fileprivate func filterPointsForLabels(fromPoints points:[Int]) -> [Int] {
    ...
    fileprivate func startAnimations(withStaggerValue stagger: Double = 0) {
        ...
        var index = 0
        for i in pointsToAnimate.lowerBound ... pointsToAnimate.upperBound {
    ...
    fileprivate func dataForGraph() -> [Double] {
        return data
    }
...
private class LabelPool {
    ...
    @objc func deactivateLabelForPointIndex(_ pointIndex: Int){
    ...
    @objc func activateLabelForPointIndex(_ pointIndex: Int) -> UILabel {
        var label: UILabel
...
private class BarDrawingLayer: ScrollableGraphViewDrawingLayer {
    ...
    fileprivate func createPath () -> UIBezierPath {
        ...
        guard let
            ...
            let data = self.graphViewDrawingDelegate?.dataForGraph()
            else {
                return barPath
        }
...
private class GradientDrawingLayer : ScrollableGraphViewDrawingLayer {
    ...
    init(frame: CGRect, startColor: UIColor, endColor: UIColor, gradientType: ScrollableGraphViewGradientType, lineJoin: String = convertFromCAShapeLayerLineJoin(CAShapeLayerLineJoin.round)) {
...
private class ReferenceLineDrawingView : UIView {
    ...
    fileprivate func boundingSizeForText(_ text: String) -> CGSize {
        return (text as NSString).size(withAttributes: [NSAttributedString.Key.font:labelFont])
    }
    ...
    // Public @objc functions to update the reference lines with any changes to the range and viewport (phone rotation, etc).
    ...
    @objc func setViewport(_ viewportWidth: CGFloat, viewportHeight: CGFloat) {
...
// Simplified easing @objc functions from: http://www.joshondesign.com/2013/03/01/improvedEasingEquations
