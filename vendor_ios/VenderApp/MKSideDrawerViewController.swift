...
public extension UIViewController {
    ...
    @objc  func isValidShopUrl(testStr:String) -> Bool {
        //abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-
        let shopUrlCharacterset = CharacterSet(charactersIn: "abcdefghijklmnopqrstuvwxyz-")
        if testStr.rangeOfCharacter(from: shopUrlCharacterset.inverted) != nil {
            return false
        }
        return true
    }
   ...
   @objc  func isValidEmail(testStr:String) -> Bool
    {
        let emailRegEx = "[\\w.-]*[a-zA-Z0-9_]@[\\w.-]*[a-zA-Z0-9]\\.(com)"//"[A-Z0-9a-z._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}"
...
open class MKSideDrawerViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    open func setDrawerState(_ state: DrawerState, animated: Bool) {
        ...
        setWindowLevel(state == .opened ? UIWindow.Level.statusBar + 1 : UIWindow.Level.normal)
    ...
    @objc final func handlePanGesture(_ sender: UIGestureRecognizer) {
        ...
        if (sender.state == .began) {
            if drawerState == .closed {
                setWindowLevel(UIWindow.Level.statusBar + 1)
            }
        }
    ...
    fileprivate func setWindowLevel(_ windowLevel: UIWindow.Level) {
