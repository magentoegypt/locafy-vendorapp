...
class RelatedProductsOptionsController: ced_VendorBaseClass,UITableViewDataSource,UITableViewDelegate,UIGestureRecognizerDelegate {
    ...
    @IBOutlet weak var totalProductsShown: UILabel!
    ...
    //values to pass throughout views
    ...
    override func viewDidLoad() {
        ...
        topLabel.text = "Related Products".localized
        leftButton.setTitle("Save".localized, for: UIControl.State());
        leftButton.addTarget(self, action: #selector(RelatedProductsOptionsController.saveFunction(_:)), for: UIControl.Event.touchUpInside);
        
        middleButton.addTarget(self, action: #selector(RelatedProductsOptionsController.filterButtonPressed(_:)), for: UIControl.Event.touchUpInside);
        //dropDownButton.addTarget(self, action: #selector(RelatedProductsOptionsController.showPageNation(_:)), for: .touchUpInside)
        
        rightButton.setTitle("Continue".localized, for: UIControl.State());
        rightButton.addTarget(self, action: #selector(RelatedProductsOptionsController.continueFunction(_:)), for: UIControl.Event.touchUpInside);
        ...
        self.relatedProductTableView.delegate = self;
        self.relatedProductTableView.dataSource = self;
   ...
   // MARK: - Table view data source
    ...
    {
        let cell = relatedProductTableView.dequeueReusableCell(withIdentifier: "longSingleCell", for: indexPath) as! LongSingleTableViewCell;
        ...
        self.relatedProCellHeight = CGFloat(235);
        ...
        for (val) in products
        {
            ...
            relatedProductView.selectionButton.addTarget(self, action: #selector(RelatedProductsOptionsController.makeProductSelected(_:)), for: UIControl.Event.touchUpInside);
            relatedProductView.selectionButton.tag = Int(val["product_id"]!)!;
            relatedProductView.selectionButton.backgroundColor = DynamicColor.secondaryColor
    ...
    @objc func fetchRelatedProductListing(_ page:Int)
    {
        ...
        let baseURL = "vproductapi/vproducts/relatedProduct/page/\(page)";
    ...
    override func recieveResponse(data:Data?,requestUrl:String?,response:URLResponse?){
        ...
        if let data = data{
            ...
            self.productCount = "Total Products : " + jsonData["data"]["count"].stringValue
//            self.totalProductsShown.text = self.productCount
    ...
    @objc func filterButtonPressed(_ sender:UIButton)
    {
        ...
        longFilterPopupView.value5.addTarget(self, action: #selector(RelatedProductsOptionsController.showProductStatusDropdown(_:)), for: UIControl.Event.touchUpInside);
        
        longFilterPopupView.value6.setTitle(value6, for: UIControl.State());
        longFilterPopupView.value6.addTarget(self, action: #selector(RelatedProductsOptionsController.showProductTypeDropdown(_:)), for: UIControl.Event.touchUpInside);
        
        longFilterPopupView.value7.setTitle(value7, for: UIControl.State());
        longFilterPopupView.value7.addTarget(self, action: #selector(RelatedProductsOptionsController.showAttributeSetDropdown(_:)), for: UIControl.Event.touchUpInside);
        ...
        longFilterPopupView.leftButton.addTarget(self, action: #selector(RelatedProductsOptionsController.applyFilterTapped(_:)), for: UIControl.Event.touchUpInside);
        
        longFilterPopupView.rightButton.setTitle("Reset", for: UIControl.State());
        longFilterPopupView.rightButton.addTarget(self, action: #selector(RelatedProductsOptionsController.resetFilterTapped(_:)), for: UIControl.Event.touchUpInside);
    ...
    @objc func addgesturesTohideView(_ v:UIView)
    {
        ...
        let tapGesture = UITapGestureRecognizer (target: self, action:#selector(RelatedProductsOptionsController.hideView(_:)))
        v.addGestureRecognizer(tapGesture);
        tapGesture.delegate=self;
        
        let upSwipe = UISwipeGestureRecognizer(target: self, action: #selector(RelatedProductsOptionsController.hideView(_:)));
        upSwipe.direction = UISwipeGestureRecognizer.Direction.up;
        v.addGestureRecognizer(upSwipe)
        upSwipe.delegate=self;
        
        let downSwipe = UISwipeGestureRecognizer(target: self, action: #selector(RelatedProductsOptionsController.hideView(_:)));
        downSwipe.direction = UISwipeGestureRecognizer.Direction.down;
        v.addGestureRecognizer(downSwipe)
        downSwipe.delegate=self;
        
        let rightSwipe = UISwipeGestureRecognizer(target: self, action: #selector(RelatedProductsOptionsController.hideView(_:)));
        rightSwipe.direction = UISwipeGestureRecognizer.Direction.right;
        v.addGestureRecognizer(rightSwipe)
        rightSwipe.delegate=self;
        
        let leftSwipe = UISwipeGestureRecognizer(target: self, action: #selector(RelatedProductsOptionsController.hideView(_:)));
    ...
    @objc func applyFilterTapped(_ sender:UIButton)
    {
        ...
        if let view = self.view.viewWithTag(181) as? ProductFilterPopupView
        {
            ...
            if(view.value5.titleLabel!.text != nil){
                value5 = view.value5.currentTitle ?? ""//.titleLabel!.text!
            }
            if(view.value6.titleLabel!.text != nil){
                value6 = view.value6.currentTitle ?? ""//.titleLabel!.text!
            }
            var val_value7 = "";
            if(view.value7.titleLabel!.text != nil){
                value7 = view.value7.currentTitle ?? ""//.titleLabel!.text!;
                val_value7 = self.sets[view.value7.currentTitle ?? ""]!;//.titleLabel!.text!
            }
            ...
            heightToUse = CGFloat(0);
//            if let val = dropDownButton.titleLabel?.text {
//                if let integerVal = Int(val){
            self.pages = 1
            loading = true
...
//            }
    ...
    @objc func resetFilterTapped(_ sender:UIButton)
    {
        ...
        self.relatedProductTableView.reloadData()
        self.view.viewWithTag(151)?.removeFromSuperview();
        self.fetchRelatedProductListing(1);
    }
    ...
    @objc func updateProductInfo(_ proceed:Bool)
    {
        ...
        if(strToPost != "")
        {
            postString += "&relatedproducts="+strToPost;
        }
