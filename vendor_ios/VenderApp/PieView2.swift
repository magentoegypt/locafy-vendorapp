...
class PieView2: UIView {
    ...
    fileprivate func commonInit() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(PieView2.handleTap(_:)))
    ...
    @objc func handleTap(_ tap: UITapGestureRecognizer) {
        ...
        if let tappedElem = pieLayer.pieElem(in: pos) {
            let newIdx = (pieLayer.values as! [PieElement]).index(of: tappedElem)
    ...
    fileprivate func animateChanges() {
        PieElement.animateChanges {
            for (idx, obj) in self.pieLayer.values.enumerated() {
