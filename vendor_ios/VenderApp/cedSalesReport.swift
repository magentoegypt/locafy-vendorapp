...
class cedSalesReport: ced_VendorBaseClass, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate, UITextFieldDelegate {
    ...
    lazy var filterView:SalesReportFilterView = {
        let view = SalesReportFilterView()
        view.orderDateFrom.delegate = self
        view.orderDateTo.delegate = self
        view.orderStatusData = orderStatusData
    ...
    override func viewDidLoad() {
        ...
        headingLabel.text = "Product Sales".localized
    ...
    @objc func loadProducts()
    {
       ...
       // self.sendRequest(url: "vreportapi/product/sales", parameters: postString)
        ...
        self.sendRequestWithData(url: "rest/V1/vreport/getSales", params: param)
    ...
    override func recieveResponse(data: Data?, requestUrl: String?, response: URLResponse?) {
        ...
        if let data = data{
            ...
            print(json)
            if(requestUrl == "rest/V1/vreport/getSales")
            {
                json["vendor_data"]["payment_status"].arrayValue.forEach{orderStatusData[$0["value"].stringValue] = $0["label"].stringValue}
                if(json["vendor_data"]["success"].stringValue == "true")
                {
                    for product in json["vendor_data"]["product"].arrayValue{
                        salesData.append(["Order Id":product["order_id"].stringValue,"Products":product["product_qty"].stringValue,"Total Amount":product["vendor_payment"].stringValue,"Tracking Number":product["tracking_number"].stringValue,"Order Date":product["order_date"].stringValue,"Status":product["order_payment_state"].stringValue,"Courier name":product["courier_name"].stringValue]);
                    }
                    print("saleds\(salesData)")
                }
                else
                {
                    if(page == 1)
    ...
    @objc func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cedSalesReportCell", for: indexPath) as! cedSalesReportCell
            let sales = salesData[indexPath.row];
            cell.orderId.text = sales["Order Id"];
            cell.products.text = sales["Products"];
        cell.orderDate.text = sales["Order Date"]?.dateConvertToCurrentTimezone("yyyy-MM-dd HH:mm:ss");
            if let delstatus = sales["Status"]{
                cell.deliveryStatus.text = orderStatusData[delstatus] ?? "";
            }
//            cell.courierName.text = sales["Courier name"];
//            cell.trackingNumber.text = sales["Tracking Number"];
            cell.sellingPrice.text = sales["Total Amount"];
    ...
    @objc func filterButtonPressed(_ sender:UIButton)
    {
        ...
        filterView.frame = CGRect(x: 0, y: 0, width: UIScreen.main.bounds.width - 40, height: 265)
