...
import magentoegypt.locafy.addons.messaging.admin_inbox.Admin_Inbox_ChatView_Adapter;
import magentoegypt.locafy.addons.messaging.admin_inbox.Admin_Inbox_Chatview_Model;
...
public class Customer_Inbox_ChatView extends Ced_MultiVendor_NavigationActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getLayoutInflater().inflate(R.layout.customer_inbox_chat_view, content, true);
        getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);
        view_url = session.getBase_Url() + "rest/V1/vmessgingapi/customerViewMessage";

        SessionManagement = new Ced_MultiVendor_VendorSessionManagement(Customer_Inbox_ChatView.this);
        postdata = new JSONObject();
        images = new JSONArray();
        notifyreciever = findViewById(R.id.notifyreciever);
        title = findViewById(R.id.chat_title);
        chat_list = findViewById(R.id.chat_list);
        chat_list.smoothScrollBy(0, 0);
        ...
        NestedScrollView customer_inbox_parent = findViewById(R.id.customer_inbox_parent);
        ...
        addfiles.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (writeresult == PackageManager.PERMISSION_GRANTED && readresult == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (ActivityCompat.shouldShowRequestPermissionRationale(Customer_Inbox_ChatView.this, Manifest.permission.READ_EXTERNAL_STORAGE) && ActivityCompat.shouldShowRequestPermissionRationale(Customer_Inbox_ChatView.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                        Toast.makeText(Customer_Inbox_ChatView.this, "Storage permission allows us to access data. Please allow in App Settings for additional functionality.", Toast.LENGTH_LONG).show();
                    } else {
                        ActivityCompat.requestPermissions(Customer_Inbox_ChatView.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE}, 2);
                    }
                }
            }
        });

        customer_inbox_parent.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener() {
            @Override
            public void onScrollChange(NestedScrollView v, int scrollX, int scrollY, int oldScrollX, int oldScrollY) {
                View view = v.getChildAt(v.getChildCount() - 1);
                int diff = (view.getBottom() - (v.getHeight() + v.getScrollY()));
                if (diff <= 30) {
                    Log.i("REpo", "nested_in" + diff);
                    if (sendrequest) {
                        ++page_num;
                        try {
                            request_page_date();
        ...
        Ced_ClientRequestResponseRest_New response = new Ced_ClientRequestResponseRest_New(new AsyncResponse() {
            ...
            public void processFinish(Object output) throws JSONException {
                ...
                if (jsonObject_data.getString("success").equals("true")) {
                    ...
                    if (firstcall) {
                        firstcall = false;
                        customerChatModelList = converter.fromJson(chatInfo.toString(), type);
                        adapter = new Admin_Inbox_ChatView_Adapter(Customer_Inbox_ChatView.this, customerChatModelList);
                        chat_list.setAdapter(adapter);
                    } else {
                        customerChatModelList = converter.fromJson(chatInfo.toString(), type);
                        adapter.list.addAll(customerChatModelList);
                    ...
                    if (firstcall) {
                        ...
                        Toast.makeText(Customer_Inbox_ChatView.this, getResources().getString(R.string.nomoredata), Toast.LENGTH_SHORT).show();
                    }
                    sendrequest = false;
                }
            }
        }, Customer_Inbox_ChatView.this, "POST", "" + postdata);
...
//                messagepostdata.put("subject", title.getText().toString());
                ...
                msg_data_obj.addProperty("id", view_id);
                msg_data_obj.addProperty("vendor_id", SessionManagement.getVendorid());
                msg_data_obj.addProperty("subject", Objects.requireNonNull(title.getText()).toString());
                msg_data_obj.addProperty("message", message_text.getText().toString());

                if (msg_data_json_array.size() > 0) {
                    msg_data_obj.addProperty("image", msg_data_json_array.toString());
                } else {
                    if (msg_data_obj.has("image"))
                        msg_data_obj.remove("image");
                }

                if (notifyreciever.isChecked()) {
                    msg_data_obj.addProperty("send_email", "true");
                } else {
                    msg_data_obj.addProperty("send_email", "false");
                }
...
//                }
            ...
            }, Customer_Inbox_ChatView.this, "POST", "" + msg_data_obj);
            response.execute(message_url);
        } else {
            Toast.makeText(Customer_Inbox_ChatView.this, R.string.messagecant, Toast.LENGTH_SHORT).show();
        }
    }
...
//                            msg_data_json_array.remove(Integer.parseInt(text_data.getTag().toString()));
                            msg_data_json_array.remove(Integer.parseInt(text_data.getTag().toString()));
...
//                    text_data.setTag(images.length());
                    text_data.setTag(msg_data_json_array.size());
                    try {
//                        imageObject.put("name", imagename[imagename.length - 1]);
//                        imageObject.put("base64_encoded_data", base64file);

                        msg_data_json_obj.addProperty("name", imagename[imagename.length - 1]);
                        msg_data_json_obj.addProperty("base64_encoded_data", base64file);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
//                    images.put(imageObject);
                    msg_data_json_array.add(msg_data_json_obj);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
