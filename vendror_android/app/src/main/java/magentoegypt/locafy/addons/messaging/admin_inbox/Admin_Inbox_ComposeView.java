...
public class Admin_Inbox_ComposeView extends Ced_MultiVendor_NavigationActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getLayoutInflater().inflate(R.layout.admin_inbox_compose_view, content, true);
        SessionManagement = new Ced_MultiVendor_VendorSessionManagement(Admin_Inbox_ComposeView.this);
        ...
        addfiles.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (writeresult == PackageManager.PERMISSION_GRANTED && readresult == PackageManager.PERMISSION_GRANTED) {
                    ...
                    if (ActivityCompat.shouldShowRequestPermissionRationale(Admin_Inbox_ComposeView.this, Manifest.permission.READ_EXTERNAL_STORAGE) && ActivityCompat.shouldShowRequestPermissionRationale(Admin_Inbox_ComposeView.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                        Toast.makeText(Admin_Inbox_ComposeView.this, "Storage permission allows us to access data. Please allow in App Settings for additional functionality.", Toast.LENGTH_LONG).show();
                    } else {
                        ActivityCompat.requestPermissions(Admin_Inbox_ComposeView.this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE}, 2);
                    }
                }
            }
        });
            ...
            Ced_ClientRequestResponseRest_New response = new Ced_ClientRequestResponseRest_New(new AsyncResponse() {
                ...
                public void processFinish(Object output) throws JSONException {
                    ...
                    if (jsonObject.getJSONObject("vendor_data").getString("success").equals("true")) {
                        Toast.makeText(Admin_Inbox_ComposeView.this, jsonObject.getJSONObject("vendor_data").getString("message"), Toast.LENGTH_SHORT).show();
                        Intent back_to_admin_inbox = new Intent(Admin_Inbox_ComposeView.this, Admin_Inbox.class);
                        back_to_admin_inbox.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        startActivity(back_to_admin_inbox);
                        finish();
                    } else {
                        Toast.makeText(Admin_Inbox_ComposeView.this, jsonObject.getJSONObject("response").getString("message"), Toast.LENGTH_SHORT).show();
                    }
                }
            }, Admin_Inbox_ComposeView.this, "POST", "" + messagepostdata);
            response.execute(message_url);
        } else {
            Toast.makeText(Admin_Inbox_ComposeView.this, R.string.messagecant, Toast.LENGTH_SHORT).show();
        }
    }
